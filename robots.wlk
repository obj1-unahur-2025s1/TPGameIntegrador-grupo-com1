...
object robotRojo {
    ...
    method usarHabilidad() {
    if (habilidad != null && puedeUsarHabilidad) {
        game.say(self, "🔥 Usando: " + habilidad.toString())
        habilidad.activar()
        puedeUsarHabilidad = false
        game.schedule(60000, { puedeUsarHabilidad = true })
    } else if (!puedeUsarHabilidad) {
        game.say(self, "⌛ Habilidad en enfriamiento...")
    } else {
        game.say(self, "❌ No tengo habilidad")
    }
}
